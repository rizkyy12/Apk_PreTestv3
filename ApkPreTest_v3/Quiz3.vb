...
Public Class Quiz3
    Public Shared QuizMain3
    Dim pilihan As String
    Dim idJawaban As String
    Dim i As Integer = 0
    Sub MunculData()
        Call Koneksi()
        lbl_id.Text = QuizMain3
        Cmd = New OleDbCommand("Select idPertanyaan, soal, KunciJawaban from T_pertanyaan where idPertanyaan = 3 ", Conn)
        Rd = Cmd.ExecuteReader
        Rd.Read()
        If Rd.HasRows Then
            lbl_no.Text = Rd.Item("idPertanyaan")
            lbl_q3.Text = Rd.Item("soal")
            lbl_huruf1.Text = Rd.Item("KunciJawaban")
        End If
    End Sub
    Sub MasukanData()
        Call Koneksi()
        Cmd = New OleDbCommand("Select idPertanyaan, idPeserta FROM T_pertanyaan, T_peserta where idPertanyaan= 3 AND idPeserta = '" & lbl_id.Text & "'", Conn)
        Rd = Cmd.ExecuteReader
        Rd.Read()
        If Rd.HasRows Then
            ...
            Dim QuizMain4 As New Quiz4
            Quiz4.QuizMain4 = lbl_id.Text
            Me.Hide()
            Quiz4.Show()
        End If
    ...
    End Sub
    Private Sub Quiz3_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Call MunculData()
    ...
    Public Sub load_data()
        Try
            ...
            Me.Refresh()
            Call Koneksi()
            Cmd = New OleDbCommand("Select jawaban FROM T_jawaban where idPertanyaan= 3 AND idPeserta = '" & lbl_id.Text & "'", Conn)
            Rd = Cmd.ExecuteReader
            Rd.Read()
            ...
            End If
            Dim NewData As String = "update T_jawaban set [Status] = '" & lbl_status.Text & "' where [idPertanyaan]= 3 AND [idPeserta] = '" & lbl_id.Text & "'"
    ...
    Private Sub btn_update_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_update.Click
        Call Koneksi()
        Dim Update As String = "update T_jawaban set [idJawaban]='" & idJawaban & "', [jawaban] = '" & pilihan & "' where [idPertanyaan]= 3 AND [idPeserta] = '" & lbl_id.Text & "'"
